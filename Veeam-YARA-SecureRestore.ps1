...
# FIX: Track whether we have already warned about Add-VBRJobLogEvent so we
# emit the warning only once per run rather than on every log call.
...
function Write-Log {
    ...
    # FIX: Add-VBRJobLogEvent does not appear in Veeam v12 or v13 PowerShell
    # reference documentation. The -Type parameter name is also unverified.
    # If the cmdlet is detected at runtime, attempt the call but surface any
    # failure as a visible warning (once) rather than silently swallowing it.
    # TODO: Verify the correct cmdlet name and parameter set for your Veeam
    # version before enabling this path in production.
    try {
        if (Get-Command Add-VBRJobLogEvent -ErrorAction SilentlyContinue) {
            try {
                ...
                if (-not $script:VBRLogEventWarned) {
                    ...
                    $warnEntry = "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] [WARNING] Add-VBRJobLogEvent failed (verify cmdlet name and -Type param for your Veeam version): $_"
                    Write-Host $warnEntry
                    Add-Content -Path $logFile -Value $warnEntry
                }
            }
        }
    } catch {}
    ...
    foreach ($scanPath in $ScanPaths) {
        ...
        foreach ($ruleFile in $yaraRules) {
            ...
            try {
                ...
                if ($completed) {
                    # FIX: Capture errors from the child job's error stream.
                    # When the YARA process itself fails (binary not found by
                    # child process, access denied, etc.), the error records
                    # land in the job error stream and were previously discarded.
                    $jobErrors = @()
                    $output = Receive-Job $job -ErrorVariable jobErrors -ErrorAction SilentlyContinue
                    Remove-Job $job -Force
                    
                    # Surface any child-job errors explicitly
                    foreach ($jobErr in $jobErrors) {
                        Write-Log "Job error for ${scanPath}: $jobErr" -Level "ERROR"
                    }
